...
```{r}
root.raw <- './noprimer'
forward.raw <- sort(list.files(root.raw, pattern='_fwd.fastq.gz', full.names = TRUE))
reverse.raw <- sort(list.files(root.raw, pattern='_rev.fastq.gz', full.names = TRUE))
...
qc.out <- filterAndTrim(forward.raw, forward.qc, reverse.raw, reverse.qc, 
                        truncLen=c(220, 160), maxEE=c(3, 4), compress=TRUE, multithread=n.threads)
qc.out
```
